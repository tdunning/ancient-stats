...
$Log: fwords.c,v $
Revision 1.6  1996/08/01 23:57:44  ted
fixed silly bug in read_and_fix_line which hasn't been tickled yet, but
which would have hurt if lines in the conf file were ever >1000 chars
long.
...
void fill_buffer(FILE *f)
{
	...
	if (buffer_fill == -1) {
	    ...
	    x_create(buf, 128*1024);
	    
	    do {
		x_resize(buf, x_length(buf) + 128*1024);
		n = fread(buf->contents+x_length(buf), 1, 128*1024, f);
		x_length(buf) += n;
	    } while (n > 0);
...
void put_bound()
{
	...
	printf("#S %d %d\n", sentence_start, buffer_point-sentence_start);
    }
    else {
	printf("#S\n");
    }
...
int main(int argc, char *argv[])
{
    ...
    error_flag = 0;
    while (!error_flag
	   && (ch = getopt(argc, argv, "DSLPF:Wtwbsvf:x")) != EOF) {
	switch (ch) {
	  case 't':
	      standard_token_output = 1;
	      break;
	  case 'x':
	      number_format = "%x";
	      break;
	...
	fprintf(stderr,
		...
		"    -t means produce Standard Token Stream output\n"

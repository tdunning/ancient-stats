...
double entropy_term(int *p, int total, int cols, int m)
{
    double sum;
...
double mutual(int single_cell, int **table, int rows, int cols)
{
    double sum;
    int *rowsums, *colsums;
    int total;
	...
	if (table[0][0] == 0) {
	    ...
	    return log((double) table[0][0] * total/ ((double) rowsums[0] * colsums[0]))/log(2.0);
	}
    }
    else {
...
int main(int argc, char **argv)
{
    ...
    error = 0;
    use_chi2_test = 0;
    while ((ch = getopt(argc, argv, "klxmbBpMv")) != EOF) {
	switch (ch) {
	case 'l':
	    use_lr_test = 1;
	    break;
    ...
    if ( !(use_chi2_test || use_mutual || use_lr_test || use_single_cell_mutual)) {
	use_chi2_test = 1;
    }
    ...
    if (error) {
	fprintf(stderr, "usage: chi2 [-lmkM] [-x] [-p] nrows ncols\n");
	fprintf(stderr,
	...
	fprintf(stderr,
		"-m means print average mutual information\n");
	fprintf(stderr,
		"-M means print single cell mutual information\n");
	fprintf(stderr,
	...
	if (use_mutual) {
	    fprintf(stderr, "computing average mutual information\n");
	}
	if (use_single_cell_mutual) {
	    fprintf(stderr, "computing single cell mutual information\n");
	}
	...
	if (use_mutual || use_single_cell_mutual) {
	    r = mutual(use_single_cell_mutual, table, rows, cols);
	    if (!use_single_cell_mutual) {
		printf("%10.5f ", r);
	    }
	    else {
